...
func makeService(t *testing.T) *MDNSService {
	return makeServiceWithServiceName(t, "hostname", "_http._tcp", "local.", []string{"Local web server"})
}

func makeServiceWithServiceName(t *testing.T, instance, service, domain string, txtInfo []string) *MDNSService {
	m, err := NewMDNSService(
		...
		"",
		80, // port
		[]net.IP{net.IP([]byte{127, 0, 0, 1}), net.ParseIP("::1")},
		//nil,
		txtInfo, // TXT
	)
...
func TestMDNSService_ServiceAddr(t *testing.T) {
	...
	// PTR map ip to full domain (reverse mapping)
	...
	// SRV map service to ip, service object contains port/priority/weight/addr
	// e.g. SRV record: _http._tcp.local. 120 IN SRV 0 0 80 hostname.local.
	// SRV used to service discovery and load balance
	if _, ok := recs[1].(*dns.SRV); !ok {
		t.Errorf("recs[1] should be SRV record, got: %v, all reccords: %v", recs[1], recs)
	}
	// A contains ipv4 address
	if v, ok := recs[2].(*dns.A); !ok {
		t.Errorf("recs[2] should be A record, got: %v, all records: %v", recs[2], recs)
	} else {
		t.Log("A record:", v.A.String())
	}
